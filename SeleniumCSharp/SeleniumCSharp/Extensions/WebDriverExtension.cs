...
namespace SeleniumCSharp.Extensions
{
    public static class WebDriverExtension
    {
        ...
        /// <returns></returns>
        public static bool IsNotDisplayed(this IWebDriver driver, By by, int timeoutInSeconds=10)
        {
            try
            {
        ...
        /// <summary>
        /// Find the first element in the page that matches the speicfied <paramref name="by"/> criteria within the specified <paramref name="timeoutInSeconds"/>.
        /// </summary>
        ...
        /// <exception cref="NoSuchElementException">Thrown when no matching element is found within the specified <paramref name="timeoutInSeconds"/>.</exception>
        ...
        /// <summary>
        /// Finds all elements on the page that match the specified <paramref name="by"/> criteria within the specified <paramref name="timeoutInSeconds"/>.
        /// </summary>
        /// <param name="driver">The <see cref="IWebDriver"/> instance to use for the search.</param>
        /// <param name="by">The <see cref="By"/> object that defines the search criteria.</param>
        /// <param name="timeoutInSeconds">The maximum number of seconds to wait for the elements to be found.</param>
        /// <returns>An <see cref="IReadOnlyCollection{T}"/> of all <see cref="IWebElement"/>s that match the <paramref name="by"/> criteria.</returns>
        /// <exception cref="NoSuchElementException">Thrown when no matching elements are found within the specified <paramref name="timeoutInSeconds"/>.</exception>
        ...
        public static IReadOnlyCollection<IWebElement> FindElements(this IWebDriver driver, By by, int timeoutInSeconds)
        {
            if (timeoutInSeconds > 0)
            {
                ...
                wait.Until(condition =>
                {
                    ...
                    {
                        throw new TimeoutException($"Timed out after {timeoutInSeconds} seconds waiting for elements with {by}", ex);
                    }
                });
        ...
        public static void ClickElementWithJs(this IWebDriver driver, IWebElement element)
        {
            ...
            WebDriverWait wait = new WebDriverWait(driver, TimeSpan.FromSeconds(Settings.DefaultWait));
